...
def build_index(documents, index_file="index.txt"):
    """
...
def calculate_tf_idf(term, doc_id, index, documents, query_term_freq=None):
    """
    ...
    tf = doc_freq
    idf = math.log(len(documents) / len(index[term]), 10) if len(index[term]) != 0 else 0  # Handle division by zero
    return tf * idf

def calculate_query_vector(query, index, documents):
    """
    ...
    for i, term in enumerate(index.keys()):
        if term in tokens:
            vector[i] = calculate_tf_idf(term, None, index, documents, query_term_freq[term])
    return vector

def calculate_document_vectors(index, documents):
    """
    ...
    for doc_id, _ in documents.items():
        vector = np.zeros(len(index))  # Initialize a vector with zeros
        for i, term in enumerate(index.keys()):
            vector[i] = calculate_tf_idf(term, doc_id, index, documents)
...
def search(query, index, documents, alpha=0.05):
    """
    ...
    """
    query_vector = calculate_query_vector(query, index, documents)
    document_vectors = calculate_document_vectors(index, documents)

    # Calculate cosine similarity between query vector and document vectors
    similarities = {doc_id: cosine_similarity(query_vector, doc_vector) for doc_id, doc_vector in document_vectors.items()}

    # Filter documents based on threshold alpha and extract relevant document IDs
    relevant_documents = [doc_id for doc_id, similarity in similarities.items() if similarity >= alpha]

    # Sort relevant documents by descending order of similarity
    ranked_documents = sorted(relevant_documents, key=lambda doc_id: similarities.get(doc_id, 0), reverse=True)

    return ranked_documents

# Define the main GUI function
def main():
    def search_wrapper(event=None):
        query = query_entry.get()
        if query:
            # Perform search
            ranked_documents = search(query, index, documents)
            # Display query
        ...
        query_entry.config(fg='grey')  # Reset text color
    ...
    def on_entry_click(event):
        if query_entry.get() == "Enter your query here":
            query_entry.delete(0, tk.END)
            query_entry.config(fg='black')  # Change text color

    def on_focus_out(event):
        if query_entry.get() == "":
            query_entry.insert(0, "Enter your query here")
            query_entry.config(fg='grey')  # Change text color

    def on_key_press(event):
        if query_entry.get() == "Enter your query here" and not event.char.isspace():  # Check if entry is empty and key pressed is not a space
            query_entry.delete(0, tk.END)
            query_entry.config(fg='black')  # Change text color
    ...
    query_entry = tk.Entry(input_frame, width=50, fg='grey')  # Set placeholder text color
    ...
    query_entry.insert(0, "Enter your query here")
    query_entry.config(fg='grey')  # Set initial text color
    query_entry.focus_set()  # Set focus to the query entry

    search_button = tk.Button(input_frame, text="Search", command=search_wrapper)
    search_button.pack(side=tk.LEFT)
    root.bind('<Return>', search_wrapper)
    ...
    # Check if query entry is empty and set placeholder text
    check_placeholder()
